...
////为了解决arm_sin_f32在0处数据飘到6.28
//float OptimizeArm_sin_f32(float rad)
//{

//    if (fabs(rad) <= 0.0001f)
//        rad = 0.0f;
//    float sin_rad = arm_sin_f32(rad);
//    return sin_rad;
//}
...
//{
//    float angle_tmp = fmod(angle_deg + 180.0f, 360.0f)-180.0f;
//    angle_tmp = fmod(angle_tmp - 180.0f, 360.0f) + 180.0f;
//    return angle_tmp - 180.0f;
//}
//float NormalizeAngleDegPositive(float angle_deg)
//{
//    float angle_tmp = fmod(angle_deg + 180.0f, 360.0f)-180.0f;
//    angle_tmp = fmod(angle_tmp - 180.0f, 360.0f) + 180.0f;
//    return angle_tmp ;
...
//        lowwer_bound = tmp;
//    }
//    if (raw_data >= upper_bound)
//        return upper_bound;
//    if (raw_data <= lowwer_bound)
//        return lowwer_bound;
//    return raw_data;
//}
//float sign(float data)									//取符号
//{
//	return  data>=0 ? 1.0f:-1.0;
//}

//float over_angle_error(float angle_error)						//180到-180的越界处理函数
//{
//	float temp = 0.0f;
//	temp = fmod(angle_error + 180.0f,360.0f)-180.0f;
//	temp = fmod(temp - 180.0f,360.0f)+180.0f;
//	return temp;
//}
//float over_angle(float angle,float angle_error)
//{
//	float temp = 12.5f * RAD2DEG;
//	if(angle + angle_error >temp) return (temp * -1.0f )+(angle + angle_error - temp);
//	else if(angle + angle_error < (-1.0f * temp)) return temp + (angle + angle_error + temp);
//	else return angle + angle_error;
//}
